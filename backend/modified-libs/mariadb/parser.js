...
class Parser extends Command {
  constructor(resolve, reject, connOpts, cmdParam) {
    ...
    this._rowIndex = 0;
    this._rowsTiming = [];
    this.opts = cmdParam.opts
      ? Object.assign({}, connOpts, cmdParam.opts)
      : connOpts;
  ...
  readResponsePacket(packet, out, opts, info) {
    switch (packet.peek()) {
      ...
      case 0xff:
        ...
        const err = packet.readError(
          info,
          opts.logParam ? this.displaySql() : this.sql,
          this.cmdParam.stack,
        );
  ...
  readOKPacket(packet, out, opts, info) {
    ...
    if (this.opts.supportBigNumbers || this.opts.insertIdAsNumber) {
      if (
        this.opts.insertIdAsNumber &&
        this.opts.checkNumberRange &&
        !Number.isSafeInteger(Number(insertId))
      ) {
        this.onPacketReceive =
          info.status & ServerStatus.MORE_RESULTS_EXISTS
            ? this.readResponsePacket
            : null;
      ...
      if (
        this.opts.supportBigNumbers &&
        (this.opts.bigNumberStrings || !Number.isSafeInteger(Number(insertId)))
      ) {
          ...
          while (subPacket.remaining()) {
            ...
            switch (type) {
              case StateChange.SESSION_TRACK_SYSTEM_VARIABLES:
                let subSubPacket;
                do {
                  subSubPacket = subPacket.subPacketLengthEncoded(
                    subPacket.readUnsignedLength(),
                  );
                  ...
                  switch (variable) {
                    case 'character_set_client':
                      ...
                      if (info.collation === undefined) {
                        this.throwError(
                          new Error("unknown charset : '" + value + "'"),
                          info,
                        );
                        return;
                      }
                    ...
                    case 'redirect_url':
                      if (value !== '') {
                        ...
                        info.redirect(
                          value,
                          this.okPacketSuccess.bind(this, okPacket, info),
                        );
                      }
                      break;
  ...
  readColumn(packet, out, opts, info) {
    this._columns.push(
      new ColumnDefinition(packet, info, this.opts.rowsAsArray),
    );
  ...
  setParser() {
    ...
    if (this.opts.typeCast) {
      for (let i = 0; i < this._columnCount; i++) {
        this._parseFunction[i] = this.readCastValue.bind(
          this,
          this._columns[i],
        );
      }
    } else {
      const dataParser = this.binary
        ? BinaryDecoder.parser
        : TextDecoder.parser;
      for (let i = 0; i < this._columnCount; i++) {
    ...
    if (this.opts.rowsAsArray) {
      ...
      this.parseRow = this.binary
        ? this.parseRowStdBinary
        : this.parseRowStdText;
  ...
  handleNewRows(row) {
    ...
    if (this._rowIndex % 1000 === 0) {
      this._rowsTiming.push({
    ...
    if (packet.peek() >= 0xfe) {
      ...
      if (
        ...
        if (
          ...
          info.redirect(
            info.redirectRequest,
            this.resultSetEndingPacketResult.bind(this, info),
          );
        } else {
  ...
  parseRowAsArray(packet) {
    ...
    const nullBitMap = this.binary
      ? BinaryDecoder.newRow(packet, this._columns)
      : null;
    for (let i = 0; i < this._columnCount; i++) {
      row[i] = this._parseFunction[i](
        packet,
        this.opts,
  ...
  parseRowNested(packet) {
    ...
    for (let i = 0; i < this._columnCount; i++) {
      ...
      row[this.tableHeader[i][0]][this.tableHeader[i][1]] = this._parseFunction[
        i
      ](packet, this.opts, this.unexpectedError, nullBitMap, i);
    }
    return row;
  }
  ...
  parseRowStdText(packet) {
    const row = {};
    for (let i = 0; i < this._columnCount; i++) {
      row[this.tableHeader[i]] = this._parseFunction[i](
        packet,
        this.opts,
        this.unexpectedError,
      );
  ...
  readLocalInfile(packet, out, opts, info) {
    ...
    let stream;
    try {
      stream = this.opts.infileStreamFactory
        ? this.opts.infileStreamFactory(fileName)
