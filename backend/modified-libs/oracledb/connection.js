...
class Connection extends EventEmitter {
  ...
  async _processBindUnit(bindInfo, bindUnit, inExecuteMany) {
    ...
    if (bindInfo.dir !== constants.BIND_IN || inExecuteMany) {
      if (bindUnit.maxSize !== undefined) {
        errors.assertParamPropValue(
          Number.isInteger(bindUnit.maxSize) && bindUnit.maxSize > 0,
          2,
          'maxSize',
        );
    ...
    if (!inExecuteMany) {
      if (bindUnit.maxArraySize !== undefined) {
        errors.assertParamPropValue(
          Number.isInteger(bindUnit.maxArraySize) && bindUnit.maxArraySize > 0,
          2,
          'maxArraySize',
        );
  ...
  async _processBindValue(bindInfo, value, options) {
    ...
    if (bindInfo.isArray) {
      bindInfo.values = transformed.concat(
        bindInfo.values.slice(transformed.length),
      );
    } else {
  ...
  async _processExecuteManyBinds(binds, bindDefs) {
    ...
    for (let i = 0; i < binds.length; i++) {
      ...
      errors.assert(
        (byPosition && Array.isArray(row)) ||
          (!byPosition && nodbUtil.isObject(row)),
        errors.ERR_MIXED_BIND,
      );
  ...
  _verifyExecOpts(options, inExecuteMany) {
    ...
    if (inExecuteMany) {
      ...
      if (options.outFormat !== undefined) {
        errors.assertParamPropValue(
          options.outFormat === constants.OUT_FORMAT_ARRAY ||
            options.outFormat === constants.OUT_FORMAT_OBJECT,
          3,
          'outFormat',
        );
      ...
      if (options.prefetchRows !== undefined) {
        errors.assertParamPropValue(
          Number.isInteger(options.prefetchRows) && options.prefetchRows >= 0,
          3,
          'prefetchRows',
        );
  ...
  async createLob(type) {
    ...
    errors.assertParamValue(
      type === types.DB_TYPE_CLOB ||
        type === types.DB_TYPE_BLOB ||
        type === types.DB_TYPE_NCLOB,
      1,
    );
    errors.assert(this._impl, errors.ERR_INVALID_CONNECTION);
    const lob = new Lob();
  ...
  encodeOSON(value) {
    ...
    return encoder.encode(
      ...
      this._impl._osonMaxFieldNameSize,
    );
  }
  ...
  async execute(sql, a2, a3) {
    ...
    if (result.resultSet !== undefined) {
      ...
      if (options.resultSet) {
        ...
        const res = await resultSet._getAllRows();
        result.rows = res.rowsFetched;
        result.rowsTiming = res.rowsTiming;
    ...
    if (result.implicitResults) {
      for (const [key, impl] of Object.entries(result.implicitResults)) {
        ...
        if (options.resultSet) {
          ...
          const res = await resultSet._getAllRows();
          result.implicitResults[key] = res.rowsFetched;
        }
      }
    }
  ...
  async executeMany(sql, bindsOrNumIters, a3) {
    ...
    if (typeof bindsOrNumIters === 'number') {
      errors.assertParamValue(
        Number.isInteger(bindsOrNumIters) && bindsOrNumIters > 0,
        2,
      );
      ...
      errors.assertParamValue(
        Array.isArray(bindsOrNumIters) && bindsOrNumIters.length > 0,
        2,
      );
...
Connection.prototype.break = nodbUtil.callbackify(
  Connection.prototype.breakExecution,
);
nodbUtil.wrapFns(
